...
class Table(Frame):
    ...
    def search_query(self, trigger=str()):
    ...
    def change_record(self, trigger=str(), entry=tuple()):
        """Renew changed line
...
class Root(Tk):
    ...
    def __init__(self, data=tuple()):
        ...
        __m_edit.add_command(label="Скрыть меню",\
                             command=lambda: self.hide_menu(widg=root_tuple))
